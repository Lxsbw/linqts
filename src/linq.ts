...
class List<T> {
  ...
  public Distinct(): List<T> {
    return this.Where(
      (value, index, iter) =>
        (Tools.isObj(value)
          ? iter.findIndex(obj => Tools.equal(obj, value))
  ...
  public ElementAt(index: number): T {
    if (index < this.Count() && index >= 0) {
      ...
      throw new Error(
        'ArgumentOutOfRangeException: index is less than 0 or greater than or Tools.equal to the number of elements in source.'
      )
    }
  }
...
class OrderedList<T> extends List<T> {
  ...
  public ThenBy(keySelector: (key: T) => any): List<T> {
    return new OrderedList(
      this._elements,
      Tools.composeComparers(this._comparer, Tools.keyComparer(keySelector, false))
    )
  }
  ...
  public ThenByDescending(keySelector: (key: T) => any): List<T> {
    return new OrderedList(
      this._elements,
      Tools.composeComparers(this._comparer, Tools.keyComparer(keySelector, true))
    )
  }
}
...
class Tools {
  ...
  /**
    ...
    */
  static equal = <T, U>(a: T, b: U): boolean =>
    Object.entries(a).every(([key, val]) =>
    Tools.isObj(val) ? Tools.equal(b[key], val) : b[key] === val
  )
